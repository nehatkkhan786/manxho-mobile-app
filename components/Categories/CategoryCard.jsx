...
const CategoryCard = ({item, selectedCat, setSelectedCat}) => {
  let backgroundColor = selectedCat == item._id ? backgroundColor= 'red' : backgroundColor=''
  return (
      <>
        <TouchableOpacity style={{ marginRight:10, marginTop:10, justifyContent:'center', alignItems:'center', borderRadius:10,}}
            ...
            <Image
                ...
                />
            <Text style={ selectedCat == item._id ? {textDecorationLine:'underline', fontWeight:'600' } :{fontSize:12} }>{item.title}</Text>
        </TouchableOpacity>
    </>
  )
}
